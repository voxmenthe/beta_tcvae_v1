...
def display_samples(model, x, vis):
    ...
    with torch.no_grad():   # added for PyTorch 0.5
        ...
        delta = torch.autograd.Variable(torch.linspace(-2, 2, 7)).type_as(zs) # for 0.5
...
def main():
    ...
    while iteration < num_iterations:
        for i, x in enumerate(train_loader):
            ...
            elbo_running_mean.update(elbo.mean().item()) # change for pytorch 0.5
            optimizer.step()
